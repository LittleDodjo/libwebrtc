...
class RTCDesktopCapturer : public RefCountInterface {
 ...
 public:
  ...
  /**
   * @brief Габарит, в который вписывается кадр перед отдачей наружу.
   *
   * Масштабирование делает видеокарта — до обратного чтения, так что чем
   * меньше габарит, тем дешевле кадр обходится по памяти и по шине. Пропорции
   * сохраняются, вверх кадр не растягивается. Ноль (по умолчанию) — отдавать
   * нативное разрешение рабочего стола.
   *
   * Работает только на пути Desktop Duplication (Windows, захват экрана).
   * Путь webrtc с обратным чтением BGRA габарит игнорирует.
   */
